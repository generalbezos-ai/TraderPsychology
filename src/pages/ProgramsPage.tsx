        ...
        {programs.map((program) => {
          ...
          return (
            <SectionCard key={program.id} title={program.name}>
              ...
              <p className="text-xs text-slate-300 mt-1">Requires {program.minSessionsRequired} prior sessions.</p>
      ...
      <SectionCard title="Program timeline">
        {timeline.length === 0 ? <EmptyState title="No active timeline" description="Enroll in a program to unlock timeline progression." /> : (
          <div className="grid grid-cols-2 md:grid-cols-7 gap-2">
            {timeline.map((item) => <div key={item.day} className="list-item text-center text-xs">Day {item.day}<div className="mt-1 text-slate-300">{item.status}</div></div>)}
          </div>
        )}
      </SectionCard>
        ...
        {programs[0].days.length === 0 ? (
          ...
          <div className="grid md:grid-cols-3 gap-3">
            {programs[0].days.map((d) => (
              <div key={d.day} className="list-item bg-slate-900/40">
